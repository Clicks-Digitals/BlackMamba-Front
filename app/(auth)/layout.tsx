...
export default async function AuthLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="dark flex min-h-screen w-full flex-col lg:flex-row">
      ...
      <div className="relative hidden w-[42%] shrink-0 flex-col justify-between overflow-hidden bg-[#000000] p-12 text-white lg:flex">
        <div
          ...
          style={{
            ...
            maskImage: "radial-gradient(ellipse 80% 70% at 30% 40%, #000000 20%, transparent 75%)",
          }}
          aria-hidden
        />
        <div
          className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full opacity-40 blur-3xl"
          style={{ background: "radial-gradient(circle, #EB0B1A 0%, transparent 70%)" }}
        />
        <div
          ...
          <ul className="space-y-3 pt-2">
            {features.map(({ icon: Icon, label }) => (
              <li key={label} className="flex items-center gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#EB0B1A]/20 ring-1 ring-[#EB0B1A]/35">
                  <Icon className="h-4 w-4 text-[#EB0B1A]" strokeWidth={1.8} />
                </span>
      ...
      {/* Form panel */}
      <div className="flex flex-1 flex-col items-center justify-center bg-[#000000] px-4 py-10 sm:px-6 lg:py-16">
